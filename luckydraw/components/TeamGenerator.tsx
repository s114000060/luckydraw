...
const TeamGenerator: React.FC<TeamGeneratorProps> = ({ employees }) => {
  ...
  const enhanceWithAI = async () => {
    ...
    setTeams(prev => prev.map((team, idx) => ({
      ...
      icebreaker: identities[idx]?.icebreaker || "What's the best part of your work day?"
    })));
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1">
            <h2 className="text-2xl font-bold text-gray-800">Auto Grouping</h2>
            <p className="text-gray-500">Organize your participants into efficient teams.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              <Shuffle size={18} /> {teams.length > 0 ? 'Reshuffle' : 'Generate'}
            </button>
            ...
            {teams.length > 0 && (
              <>
                ...
                >
                  ...
                  {isEnhancing ? 'Thinking...' : 'AI Team Names'}
                </button>
                <button
                  onClick={downloadCSV}
      ...
      {teams.length === 0 ? (
        <div className="bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 p-20 flex flex-col items-center justify-center text-gray-400 text-center">
          ...
          <p>Select a group size and click Generate to see the results.</p>
        </div>
      ) : (
                ...
                {team.icebreaker && (
                  <div className="mt-4 p-3 bg-amber-50 rounded-xl border border-amber-100">
                    <div className="flex items-center gap-1.5 text-amber-700 font-semibold text-xs uppercase mb-1">
                      <Sparkles size={12} /> Team Icebreaker
                    </div>
